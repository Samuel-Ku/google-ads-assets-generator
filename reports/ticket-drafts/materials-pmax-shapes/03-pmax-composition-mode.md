...
This slice delivers the primary text pair end to end. The UI and export count must clearly describe that scope until ticket 4 adds automatic A/B variants; do not imply that all text combinations are already rendered.
...
Coordination: this ticket and ticket 5 may start independently. Whichever lands second verifies all then-supported native shapes through PMax mode switching, Team Template reuse and real export. This is an integration responsibility, not a blocking edge.
